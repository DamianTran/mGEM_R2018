  ...
  for(y in 1:rowNum){
    for(x in 1:colNum){
      for(i in 1:nchar(dataset[y,x])){
        randNum = runif(1)
        if(randNum < frequency/2){ # Draw for a chance or a point mutation
          randNum = runif(1)
          ...
          randNum = runif(1) # Draw again for a chance for a random insertion or deletion
          if(randNum < frequency/2){
            randNum = runif(1)
            if(randNum >= 0.5) dataset[y,x] = # Truncation
                paste(substr(dataset[y,x], 1, i-1), 
                      substr(dataset[y,x], i+1, 
                             nchar(dataset[y,x])), 
                      sep = "")
            else{
              randNum = runif(1)
              randNT = character(1)
              if((randNum >= 0) & (randNum < 0.25)) randNT = "A"
              else if((randNum >= 0.25) & (randNum < 0.5)) randNT = "T"
              else if((randNum >= 0.5) & (randNum < 0.75)) randNT = "C"
              else randNT = "G"
              dataset[y,x] = 
                paste(substr(dataset[y,x], 1, i), 
                      randNT,
                      substr(dataset[y,x], i+1, 
                             nchar(dataset[y,x])), 
                      sep = "")
...
NGSgen = function(ncol, nrow, sequence, mutantFreq, quality){
  ...
  # .. introduce a non-specific species permutator
  
  # Add additional permutation functions
...
# enter "source("stats.R")" for a stats example
